...
# Input文字：左移，远离图形
ax.text(-0.8, -1.0, 0.05, 'Input', fontsize=16, ha='right', weight='medium')

# ConvLSTM1
draw_box(x0=0.2, y0=0.2, z0=0.4, dx=2.6, dy=1.6, dz=0.1, color=color_conv1)
# ConvLSTM1文字：固定最左侧，不遮挡长方体
ax.text(-0.8, -1.0, 0.45, r'$ConvLSTM_1$', fontsize=16, ha='right', weight='medium')

# ConvLSTM2
draw_box(x0=0.4, y0=0.4, z0=0.8, dx=2.2, dy=1.2, dz=0.1, color=color_conv2)
# ConvLSTM2文字：固定最左侧，不遮挡长方体
ax.text(-0.8, -1.0, 0.85, r'$ConvLSTM_2$', fontsize=16, ha='right', weight='medium')

# 编码网络标题：大幅上移，远离上层长方体
ax.text(1.5, 1.2, 1.1, 'Encoding Network', fontsize=18, ha='center', weight='bold', color=line_color)
...
# ConvLSTM3文字：固定最右侧，不遮挡长方体
ax.text(9.0, -1.0, 0.45, r'$ConvLSTM_3$', fontsize=16, ha='left', weight='medium')

# ConvLSTM4
draw_box(x0=6.2, y0=0.4, z0=0.8, dx=2.2, dy=1.2, dz=0.1, color=color_conv4)
# ConvLSTM4文字：固定最右侧，不遮挡长方体
ax.text(9.0, -1.0, 0.85, r'$ConvLSTM_4$', fontsize=16, ha='left', weight='medium')

# Prediction
draw_box(x0=6.4, y0=0.6, z0=1.2, dx=2.0, dy=1.0, dz=0.1, color=color_pred, edgecolor='#006D8F', linewidth=2)
# Prediction文字：右移+上移，不遮挡长方体
ax.text(7.4, 2.0, 1.25, 'Prediction', fontsize=16, ha='center', color='#006D8F', weight='bold')

# 解码网络标题：上移+右移，远离上层长方体
ax.text(7.3, 1.2, 1.1, 'Forecasting Network', fontsize=18, ha='center', weight='bold', color=line_color)
